...
```
- 加法的算法步骤如下
> a 如果自运算符是对象，先自动转换成原始类型的值(即先执行对象的valueOf方法，如果结果还不是原始类型的值，在执行toString方法；如果对象是Date实例，则先执行toString方法)<br>
> b 两个运算子都是原始类型后，只要有一个运算子是字符串，则连个运算子都转化为字符串，执行字符串链接运算<br>
> c 否则两个运算子都转化为数值，执行加法运算

- 如果加法运算符左边是一个字符串，导致右边的运算子都先转化为字符串
```js
  ...
  '1' + [1]  //"11"
```
- 这种由于参数不同，而改变自身行为现象，叫做"重载"，由于加法运算符是运行时决定到底执行哪种运算,字符串位置不同可以引起结果不同
```js
  '3' + 4 + 5 //"345"
  3 + 4 + '5' //"75"
```
...
```js
 ...
  String([1,2]) + String([3])
  //'1,2' + '3'
```
- 加法运算符一定有左右两个运算子，如果只有邮编一个运算子，就是另一种运算符，叫做"数值运算符"
```js
  + - 3 // 等同于+(-3)
  + 1 + 2 //等同于+(1 + 2)
  + '1' //1
...
```
- 加法运算符以外其他运算符（减法、除法和乘法），都不会发生重载，他们的规则是：所有的运算子一律转换为数值，在进行相应的数学运算
```js
  1 - '1' //0
  1 * '2' //2
  1 / 2 //0.5
```
- 由于加法运算符与其他运算符的区别，会导致一些意外结果
```js
  var now = new Date();
  typeof(now + 1) //"string"
  typeof(now - 1) //"number"
...
```
#### 2.2自增和自减运算符
- 自增和自减运算符，是一元运算符，只需要一个运算子，他们的作用是将运算子首先转换为数值，然后加上1或者减去1，他们会修改原始变量
```js
  var x = 1;
  ++x  //2
  x //2
  ...
  x //1
```
- 自增和自减运算符需要注意的是：放在变量之后，会返回变量运算前的值，在进行自增/自减运算，放在变量之前，会先进行自增/自减运算，再返回变量运算后的值
```js
  var x = 1;
  var y = 1;
...
```
#### 2.3数值运算符，负数值运算符
- 数值运算符（+）同样适用加好，但是加法运算符是二元运算符
- 数值运算符的作用在于可以将任何值转化为数值（与Number函数的作用相同）
```js
  +true //1
  +[] //0
  +{} //NaN
  ...
  -(-x) //1
```
- 上面最后一行代码的圆括号不能省略，否则会变成递减运算符
- 数值运算符和负数运算符，都会返回一个新值，而不会改变陨石变量的值
### 3.赋值运算符
- 赋值运算符用于给变量赋值，最常见的是等号（=）
### 4.比较运算符
- 比较运算符用于比较两个值，然后返回每一个布尔值，表示是否满足条件
```js
  2 < 6 //true
```
- JavaScript一共提供8个比较运算符
> == 相等
> === 严格相等
> != 不相等
> !== 不严格相等
...
> 他们的区别是相等运算符（==）比较两个值是否相等，严格相等运算符（===）比较他们是否问‘同一个值’
